...
/*/home/chetan/frappe-bench/apps/lms/frontend/src/components/CourseCardOverlay.vue*/
...
<Button
  ...
  size="md"
  class="w-full"
  @click="goToFirstIncompleteLesson"
>
  <span>{{ __('Continue Learning (Auto)') }}</span>
</Button>
...
async function goToFirstIncompleteLesson() {
  try {
    const outlineResource = createResource({
      url: 'lms.lms.utils.get_course_outline'
    })

    const response = await outlineResource.submit({
      course: props.course.data.name
    })

    const outline = response.message

    // Guard: check it's an array
    if (!outline || !Array.isArray(outline)) {
      toast.error(__('Invalid course outline received'))
      return
    }

    let targetLesson = null

    // Step 1: Find first incomplete
    ...
    // Step 2: Fallback to first if all complete
    if (!targetLesson && outline.length && outline[0].lessons && outline[0].lessons.length) {
      targetLesson = outline[0].lessons[0]
    }

    if (targetLesson) {
      const [chapterNumber, lessonNumber] = targetLesson.number.split('.')
      router.push({
        name: 'Lesson',
        params: {
          courseName: props.course.data.name,
      ...
      toast.error(__('No lessons found for this course'))
    }
  } catch (error) {
    console.error(error)
