/*  /home/chetan/frappe-bench/apps/lms/frontend/src/pages/CertifiedParticipants.vue */
...
console.log('[Init] Logged-in user:', user)
...
const participants = createListResource({
  ...
  onSuccess(data) {
	...
	const userEmail = user; // e.g. user = 'lmstudent@gmail.com';
...
const updateParticipants = () => {
	updateFilters()
  participants.update({ filters: filters.value })
  participants.reload({ force: true })
}


const updateFilters = () => {
	filters.value.member = user.email // always filter by user
